...
flags.DEFINE_string("saved_model_dir", None, "model saved directory")
FLAGS = flags.FLAGS
...
def main(unused_argv):
  ...
  with tf.device(
    ...
    if is_chief & (FLAGS.saved_model_dir is not None):
      sess.graph._unsafe_unfinalize()
      export_path = os.path.join(tf.compat.as_bytes(FLAGS.saved_model_dir))
      print('Exporting trained model to', export_path)
      serialized_tf_example = tf.compat.v1.placeholder(tf.string, name='tf_example')
      values, indices = tf.nn.top_k(y, 10)
      tensor_info_x = tf.compat.v1.saved_model.utils.build_tensor_info(x)
      tensor_info_y = tf.compat.v1.saved_model.utils.build_tensor_info(y)
      table_constant = tf.constant([str(i) for i in range(10)])
